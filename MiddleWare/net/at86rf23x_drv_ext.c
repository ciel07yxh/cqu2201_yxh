...
//    trx_bit_write(SR_IRQ_POLARITY, 1); // 如果这配置，则为低电平有效
    ...
    ** Step 6, 扩展工作模式配置
    */  
    trx_bit_write(SR_MAX_FRAME_RETRIES, 1);
    ...
    ** 3--------->2000Kbps(芯片扩展支持)
    */  
    //trx_bit_write(lowpanif, SR_OQPSK_DATA_RATE, 3);
...
//                                          lowpanif->addr); 
//    lowpanif->radio_driver->set_channel(lowpanif,
//                                         lowpanif->channel);
    ...
    */
//    trx_bit_write(lowpanif, SR_CLKM_SHA_SEL, 0);
//    trx_bit_write(lowpanif, SR_CLKM_CTRL, 0);

    /*
    ** Step 14, 电池监控配置, 设置低压报警，低于2.3V报警，具体请参看数据手册，寄存器BATMON，地址0x11
    */
//    trx_bit_write(lowpanif, SR_BATMON_HR, 0);
//    trx_bit_write(lowpanif, SR_BATMON_VTH, 0xc);

    /*
    ** Step 15, Calibration配置
    */
//    trx_bit_write(lowpanif, SR_FTN_START, 1);
//    usdelay(((sys_clock_get() * 10000)/(2*1000000)) + 1);
...
//  trx_state_set(lowpanif, STATE_BUSY_TX);
...
//     while(trx_bit_read(lowpanif, SR_CCA_DONE) == 0);
//     return ((trx_bit_read(lowpanif, SR_CCA_STATUS)));
}
...
**  定义RF驱动结构体
*********************************************************************************************************/
const struct radio_driver at86rf231_rf_driver =
{
    init,
    prepare,
    transmit,
